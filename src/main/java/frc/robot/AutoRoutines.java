...
public class AutoRoutines {
    ...
    public Command HtoI() {
        ...
        routine
                ...
                .onTrue(
                        Commands.parallel(
                                Commands.waitUntil(this::atBranch)
                                        .andThen(
                                                Commands.parallel(
                                                        superstructure.elevator
                                                                .setExtension(ElevatorConstants.A2 + 0.25),
    ...
    public Command Itobarge() {
        ...
        routine
                ...
                .onTrue(
                        Commands.parallel(
                                Commands.waitUntil(this::atBarge).andThen(
                                        superstructure.elevator.setExtension(ElevatorConstants.AN)),
                                Commands.waitUntil(() -> superstructure.elevator.isNearExtension(ElevatorConstants.AN))
    ...
    public boolean atReef() {
        return drive
                .getPose()
                .getTranslation()
                .getDistance(AllianceFlipUtil.apply(FieldConstants.Reef.center)) < ElevatorConstants.reefRaiseDistance;
    }
    ...
    public boolean atAlgae() {
        return AutoAlign.isInTolerance(
                drive.getPose(), AllianceFlipUtil.apply(
                        Reef.algaeIntake[AutoAlign.getBestFace(drive.getPose(), 0, 0)]));
    }

    public boolean atBarge() {
        return Math.abs(7.75 - AllianceFlipUtil.applyX(drive.getPose().getX())) < 0.125;
    }
